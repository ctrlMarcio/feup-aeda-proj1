...
RentalContract *ContractManager::build(IUser *user, Rental &rental) {
    return new RentalContract(Date(), user, RENTAL, rental);
}

TransferContract *ContractManager::build(IUser *user, Offer &offer) {
    return new TransferContract(Date(), user, TRANSFER, offer);
}
...
BST<Contract *, contract_less_than> &ContractManager::getContracts() {
    return contracts;
}
